...
class Message():
        ...
        if self.message.get_content_maintype() == "multipart":
            for content in self.message.walk():       
                if content.get_content_type() == "text/plain":
                    try:
                        self.body = content.get_payload(decode=True).decode(content.get_content_charset())
                    except:
                        self.body = content.get_payload(decode=True)
                if content.get_content_type() == "application/ics":
                    self.calender = content.get_payload(decode=True)
    ...
    def fetch(self):
        if not self.message:
            response, results = self.gmail.imap.uid('FETCH', self.uid, '(BODY.PEEK[] FLAGS X-GM-THRID X-GM-MSGID X-GM-LABELS ENVELOPE)')
